			...
			_openDeleteModal: function () {
				...
				for (var j in columnDefs) {
					if (columnDefs[j].type.indexOf("hidden") >= 0) {
						...
						data += "<div class='row mb-3 mb-sm-4'><label class='col-lg-3 col-form-label form-label text-left text-lg-right' for='"
							+ that._quoteattr(columnDefs[j].name)
							+ "'>"
							+ columnDefs[j].title
							+ ":</label> <div class='col-lg-9 d-flex align-items-center'><div class='border rounded w-100 px-3 py-2 bg-faded'><input type='hidden' id='"
				...
				$(selector).on('show.bs.modal', function () {
					var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">' + that.language.modalClose + '</button>' +
						'<button type="submit"  data-content="remove" class="btn btn-danger" id="deleteRowBtn"><i class="fal fa-times mr-1"></i>  ' + that.language.delete.button + '</button>';
					...
					if (modalContent.parent().is('form')) {
						...
						modalContent.wrap("<form class='w-100' name='" + formName + "' id='" + formName + "' role='form'></form>");
					}
				});
			...
			createDialog: function(columnDefs, title, buttonCaption, closeCaption, buttonClass, formName) {
				...
				for (var j in columnDefs) {
					...
					if (columnDefs[j].type.indexOf("hidden") >= 0) {
						...
						data += "<div class='form-group row' id='alteditor-row-" + this._quoteattr(columnDefs[j].name) +"'>"
						...
						if (columnDefs[j].type.indexOf("readonly") >= 0) {
							...
							data += "<input type='text' readonly  id='"
								...
								+ "' placeholder='"
								+ this._quoteattr(columnDefs[j].placeholderMsg)
								+ "' style='overflow:hidden'  class='form-control' value=''>";
						}
						// Adding select-fields
							...
							data += "<select class='form-control" + (columnDefs[j].select2 ? ' select2' : '')
								...
								+ "' placeholder='" + this._quoteattr(columnDefs[j].placeholderMsg)
						...
						data += "<span id='" + this._quoteattr(columnDefs[j].name) + "label"
								+ "' class='errorLabel help-block text-danger'></span>";
